...
hosts/
  common.nix              what every host shares: boot, nix, users, locale,
                          sound, X + dwm, docker, tailscale
  <host>/configuration.nix        host deltas: keymap, GPU facts, (nixbox1) VMs
  <host>/home.nix                 host facts: wallpaper, xrandr line
  <host>/hardware-configuration.nix  generated by nixos-generate-config
modules/
  nvidia.nix              PRIME offload; the host fills in vars.nvidia
  home.nix                apps, shell, and what the session starts
overlays/vendored.nix     the local forks in src/ (one place, not four)
src/                      vendored C sources (dwm, st, slock, slstatus)
patches/                  where each local C change came from (see patches/README.md)
pics/                     wallpapers
```

Eleven `.nix` files, two of them generated. The flake exports
...
| `modules/nvidia.nix`, `modules/home.nix`       | one stack, every machine | connector names, hostnames |
| `hosts/common.nix`                             | everything shared        | specific hosts          |

**2. Host facts flow through options, not hardcoding.** `modules/home.nix`
...
2. `hosts/<name>/configuration.nix` — import it, set the keymap, fill in `vars.nvidia`, add anything only this machine runs.
...
**once**, in `overlays/vendored.nix`, which `flake.nix` applies to the package
set before any module can say `pkgs.dwm`:
